...
Content teams at FlyRank manage refresh decisions across a large, growing library of published pages, but reviewer time is limited — teams can't manually audit every page every cycle. This project originated from that real constraint: FlyRank needed a way to identify which pages, out of a large pool, were most worth a reviewer's limited time. This paper asks: which pages should a content/SEO team prioritize for refresh review, given a decline signal, to support that decision? Using ~9.8 million rows of production search performance data from the FlyRank internship warehouse, we trained a Random Forest classifier on December 2025 features (impressions, clicks, search position, age) to predict which pages would show declining visibility by March 2026. The model achieved a Precision@50 of 1.000 and AUC of 0.933–0.935, compared to a rule-based baseline's Precision@50 of 0.000 and AUC of 0.627. These results support using the model as a decision-support tool for refresh prioritization at FlyRank's scale, though the label is a proxy for decline rather than a causal signal, and the model has not been validated on any window beyond this single historical panel.

## 1. Introduction / Problem Statement

This work is a case study built directly from a FlyRank content operations problem: as FlyRank's client base grows, so does the volume of published content each client needs maintained. A recurring bottleneck emerged — content/SEO reviewers have far more potentially-declining pages than they have hours to check, and deciding which pages deserve attention first has historically relied on simple heuristics like page age, which this paper shows performs no better than random guessing at the top of the queue (Precision@50 of 0.000). This is the lane the FlyRank ML Internship dataset was built to explore: can a learned model do meaningfully better than that heuristic, using only signals reviewers already have access to?
